...
struct communicate {											//每个初始化成功后通信设备的拥有的结构体对象
	...
	int (*reback[3])(int fd);                                   //用来存储各个函数的入口地址，完成重新初始化、发送消息等 ：在这里固定为 0：初始化  1：发操作  2：用户定义的其他操作 
	int timecount;												//生存计数器
	int timeout; 												//超数阈值，如若超过阈值，就认定为此通信设备连接中断，即开始重新初始化
};
struct communicateNode {
	struct communicate* com;
...
public:
	...
	vector<vector<int>>  getALLIfd();							//获取全部通信设备id号、fd号及按照id号从大到小顺序返回
	int getSuccessFd();
	int getSuccessId();
	bool setEnable(int id);										//将注册成功的通信设备描述置为使能
	int getSize();
	bool isExist(int id);
	communicaManage();
	~communicaManage();
